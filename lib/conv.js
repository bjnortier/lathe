    ...
    var bspToBrep = function(bsp, shpclass) {
        ...
        var accumulateBreps = function(ancestors, v) {
            if (v instanceof BSP.Node) {
                ...
                if (shp) {
                    var classification = classifyWrtBSP([shp], v);
                    breps = breps.concat(classification.on);
                }

                accumulateBreps(ancestors.concat([{node: v, side: 'back'}]), v.back);
                accumulateBreps(ancestors.concat([{node: v, side: 'front'}]), v.front);
            }
        }
