...
define([
  ...
  var accumulateBreps = function(acc, v) {
    var acc1 = acc;
    if (v instanceof BSP.Node) {
      var classification = classifyWrtBSP([v.shp], v);
      if (classification.on.length) {
        acc1 = acc.concat(classification.on);
      } 
      acc1 = acc1.concat(accumulateBreps(acc, v.back));
      acc1 = acc1.concat(accumulateBreps(acc, v.front));
    }
    return acc1;
  }
