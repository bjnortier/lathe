...
define(['lib/vector3', 'lib/plane3d', 'lib/world3d', 'lib/vertex3d'], function(Vector3, Plane3D, world3d, Vertex3D) {
    ...
    Polygon3D.prototype.isConvex = function() {
        try {
            ...
            for (var i = 0; i < vectors.length; ++i) {
                edges[i] = vectors[(i+1) % vectors.length].sub(vectors[i]);
            }
            var cross012 = edges[1].cross(edges[0]);
            for (var i = 3; i <= vectors.length + 1; ++i) {
                var cross = edges[(i-1) % vectors.length].cross(edges[i-2]);
    ...
    Polygon3D.prototype.isConvex2 = function() {
        try {
            var vertices = this.toVertices();
            if (vertices.length === 3) {
                return true;
