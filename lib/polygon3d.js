...
define(
  ...
  // The plane is created from the normal and one of the points (a)
  // The bounding planes are created from two points and a point
  // at a distance 1 form the plane (and hence perpendicular to the plane)
  Polygon3D.from3PointsAndNormal = function(a,b,c,n) {
    var d = (a.x*n.x + a.y*n.y + a.z*n.z);
    var s =  new Plane3D(n.x, n.y, n.z, d);

    var pairs = [[a,b], [b,c], [c,a]];
    var boundingPlanes = pairs.map(function(pair) {
      var w = pair[0].add(n.normalize());
      return new Plane3D.fromPoints(pair[0], pair[1], w);
    });
