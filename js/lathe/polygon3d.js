...
define(
  ...
  Polygon3D.prototype.splitByUncached = function(h) {
    ...
    if (this.s.isCoincident(h)) {
      ...
      var eliminateDuplicatePlanes = function(planes) {
        var uniquePlanes = [];
        var l = planes.length;
        for (var i = 0; i < l; ++i) {
          if (!planes[i].equals(planes[(i+1)%l])) {
            uniquePlanes.push(planes[i]);
          }
        }
        return uniquePlanes;
      }
      ...
      var front;
      if (frontOutputPlanes.length > 0) {
        var uniquePlanes = eliminateDuplicatePlanes(frontOutputPlanes);
        if (uniquePlanes.length > 2) {
          var p = new Polygon3D(this.s, uniquePlanes);
          front = p.isValid() ? p : undefined;
        }
      }
      
      var back;
      if (backOutputPlanes.length > 0) {
        var uniquePlanes = eliminateDuplicatePlanes(backOutputPlanes);
        if (uniquePlanes.length > 2) {
          var p = new Polygon3D(this.s, uniquePlanes);
